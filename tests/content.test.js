...
import { validateProjectShape, validateProjectTranslations } from '../src/js/validation/content.js';
...
test('validateProjectShape reports missing required fields', () => {
  const errors = validateProjectShape({ id: 'test', tags: ['x'], decisions: ['a', 'b'], proof: ['a', 'b'], meta: [['k', 'v'], ['k2', 'v2']], links: [['GitHub', 'https://github.com', true]] }, 'en');
  assert.ok(errors.some((e) => e.includes('title')));
});

test('validateProjectShape reports insufficient decisions', () => {
  const minimal = {
    id: 'x', title: 'X', badge: 'B', category: 'C', shortPitch: 'p',
    shortDescription: 'd', status: 's', focus: 'f', stack: 'st',
    tags: ['t'], quote: 'q', summary: 's', context: 'c',
    architecture: 'a', decisions: ['only one'], proof: ['a', 'b'],
    meta: [['k', 'v'], ['k2', 'v2']], links: [['G', 'https://g.com', true]],
  };
