...
export function applyStaticTranslations(language) {
  ...
  const htmlMap = {
    ...
    "about-copy-block": translations[language].about.paragraphs.map((paragraph) => `<p>${paragraph}</p>`).join(""),
  };
  ...
  document.getElementById("about-quote-line").textContent = translations[language].about.quote;
  document.getElementById("play-placeholder-copy-text").textContent = t("play.placeholderCopy");
  document.getElementById("play-points-list").innerHTML = translations[language].play.points
  ...
  document.getElementById("strength-list").innerHTML = stack.strengths.map((item) => `<li>${item}</li>`).join("");
  ...
  document.getElementById("modalCloseBtn").setAttribute("aria-label", t("modal.closeAria"));
}
