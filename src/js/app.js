...
import { setupActiveNav, setupScrollBar, setupSectionReveal } from "./features/navigation.js";

const supportedLanguages = Object.keys(translations);
let currentLanguage = ensureLanguage(localStorage.getItem("portfolio-lang") || "en", supportedLanguages);
...
function renderLanguageDependentUI() {
  ...
  setupPlayableDemo(t);
  fetchGitHubStats(currentLanguage, t);
}
...
function setLanguage(language) {
  ...
  if (modalOverlay.classList.contains("open")) closeProjectModal();
}
...
function setupProjectModalInteractions() {
  document.addEventListener("click", (event) => {
    ...
    if (trigger) openProjectModal(currentLanguage, trigger.dataset.projectId, t);
  });
